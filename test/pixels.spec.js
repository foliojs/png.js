...
const notImplemented = ['interlaced-rgb-8bit.png', 'interlaced-rgb-16bit.png', 'interlaced-pallete-8bit.png', 'interlaced-grayscale-8bit.png']

const files = fs.readdirSync('test/images').filter(fileName => !notImplemented.includes(fileName))

async function getPixels(Ctor, fileName) {
  const image = new Ctor(fs.readFileSync(`test/images/${fileName}`));  
  return new Promise(resolve => {
    Ctor === PNGNode ? image.decodePixels(resolve) : resolve(image.decodePixels())
  })
}
